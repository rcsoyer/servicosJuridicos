...
export class AssuntoService implements BasicService<Assunto> {
    ...
    private readonly resourceUrl = this.baseApiURL + 'assunto';

    constructor(private http: HttpClient) { }

    create(assunto: Assunto): Observable<EntityResponseType> {
        return this.http
                   .post<Assunto>(this.resourceUrl, assunto, { observe: 'response' });
    }

    update(assunto: Assunto): Observable<EntityResponseType> {
        return this.http
                   .put<Assunto>(this.resourceUrl, assunto, { observe: 'response' });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
                   .get<Assunto>(`${this.resourceUrl}/${id}`, { observe: 'response' });
    }

    query(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http
                   .get<Assunto[]>(this.resourceUrl, { params: options, observe: 'response' });
    }
